            ...
            <tbody>
             <?php
              ...
              if ($conn->connect_error) {
                ...
                $sql="select imie, nazwisko, e_mail ,sum(zamowione_produkty.ilosc) as suma from
                (klient inner join zamowienie on zamowienie.id_klienta=klient.id and id_stan!=1";
                ...
                $sql .=" )
                inner join zamowione_produkty on zamowione_produkty.id_zamowienia=zamowienie.id
                group by klient.id order by suma desc limit 3;";
               ...
               // echo $sql;
                ...
                if($result->num_rows>0){
                   while($row=$result->fetch_assoc()){
                    ...
                    echo "<tr ><td> {$row['imie']}</td><td> {$row['nazwisko']}</td><td> {$row['e_mail']}</td><td> {$row['suma']} </td>";
                         echo "</tr>";
                    }
               }
