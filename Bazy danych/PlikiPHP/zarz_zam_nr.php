...
<body>
    ...
    <?php
       ...
       ?>
        ...
        echo " <form action='zmien_oplacone.php' method='post'>
            <select name='oplata' >";
        if($row['czy_oplacone']==0)echo"<option value='false'>nie</option><option value='true'>tak</option>"; 
        else echo"<option value='true'>tak</option><option value='false'>nie</option>";        
        echo   "</select>
        ...
        <p1><b>Miejscowosc: </b> {$row2['miejscowosc']} </p1><br/>
         ...
         echo "<br/><br/><h3>Zmień stan zamówienia:<h3> 
         <form action='zmien_stan_zam.php' method='post'>
         <select name='stan'>";
         $result2=$conn->query("select id,stan from stan_zamowienia where id not in (1)");
         if($result2->num_rows>0){
            while($row2=$result2->fetch_assoc()){
                if($row2['stan']==$row['stan'])echo "<option selected='selected' value=\"{$row2['id']}\">{$row2['stan']} </option>";
                else echo "<option value=\"{$row2['id']}\">{$row2['stan']} </option>";
            }
        }
